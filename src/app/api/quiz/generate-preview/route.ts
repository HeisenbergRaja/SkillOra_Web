...
export async function POST(request: Request) {
  try {
    ...
    const { prompt, questionCount: reqCount, title } = body;
    ...
    while (allQuestions.length < questionCount && loopSafetyCounter < MAX_LOOPS) {
      ...
      const userPrompt = buildUserPrompt(prompt, questionsToGenerate);

      const sysTokens = Math.ceil(INSTRUCTIONS.length / 4);
      const userTokens = Math.ceil(userPrompt.length / 4);
      const finalTokens = sysTokens + userTokens + 10; // estimate
    ...
    if (allQuestions.length !== questionCount) {
      ...
      return NextResponse.json({ success: false, error: 'QUESTION_COUNT_MISMATCH' }, { status: 500 });
    }
    ...
    const finalQuiz: FinalQuiz = {
      ...
      quizTitle: `${title || 'Skill'} Final Assessment`,
    ...
    return NextResponse.json({ success: true, quiz: finalQuiz });
    ...
    return NextResponse.json({ success: false, error: error.message || 'JSON_INVALID' }, { status: 500 });
  }
}
